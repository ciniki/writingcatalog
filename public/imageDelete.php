...
function ciniki_writingcatalog_imageDelete(&$ciniki) {
    ...
    $rc = ciniki_writingcatalog_checkAccess($ciniki, $args['business_id'], 'ciniki.writingcatalog.imageDelete', 0); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
	...
	if( !isset($rc['item']) ) {
		...
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2452', 'msg'=>'Gallery image does not exist'));
	}
	$item = $rc['item'];
