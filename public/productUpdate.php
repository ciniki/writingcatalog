...
//
// writingcatalog_id:	(optional) The new ID of the writingcatalog item the product is attached to.
//
...
// <rsp stat='ok' />
//
function ciniki_writingcatalog_productUpdate(&$ciniki) {
    //  
    ...
    $rc = ciniki_writingcatalog_checkAccess($ciniki, $args['business_id'], 'ciniki.writingcatalog.productUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
	...
	//
	$strsql = "SELECT id, writingcatalog_id "
	...
	if( !isset($rc['item']) ) {
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2468', 'msg'=>'Product does not exist'));
	}
	$product = $rc['item'];
	...
	if( isset($args['name']) ) {
		...
		$strsql = "SELECT id, name, permalink "
			...
			. "AND writingcatalog_id = '" . ciniki_core_dbQuote($ciniki, $product['writingcatalog_id']) . "' "
		...
		if( $rc['num_rows'] > 0 ) {
			return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2469', 'msg'=>'You already have a product with this name, please choose another name'));
		}
	}
	...
	return ciniki_core_objectUpdate($ciniki, $args['business_id'], 'ciniki.writingcatalog.product', $args['product_id'], $args, 0x07);
}
?>
