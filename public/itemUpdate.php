...
function ciniki_writingcatalog_itemUpdate(&$ciniki) {
    ...
    $rc = ciniki_writingcatalog_checkAccess($ciniki, $args['tnid'], 'ciniki.writingcatalog.itemUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    if( isset($args['title']) ) {
        ...
        if( $rc['num_rows'] > 0 ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.writingcatalog.22', 'msg'=>'You already have an item with this title, please choose another title.'));
        }
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.writingcatalog.item',
