...
// <rsp stat='ok' />
//
function ciniki_writingcatalog_productDelete(&$ciniki) {
    //  
    ...
    $rc = ciniki_writingcatalog_checkAccess($ciniki, $args['business_id'], 'ciniki.writingcatalog.productDelete'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
	...
	//
	$strsql = "SELECT uuid FROM ciniki_writingcatalog_product "
	...
	if( !isset($rc['product']) ) {
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2448', 'msg'=>'Unable to find existing product'));
	}
	...
	foreach($ciniki['business']['modules'] as $module => $m) {
		...
		if( $rc['stat'] == 'ok' ) {
			...
			$rc = $fn($ciniki, $args['business_id'], array(
				'object'=>'ciniki.writingcatalog.product', 
				'object_id'=>$args['product_id'],
				));
			if( $rc['stat'] != 'ok' ) {
				return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2449', 'msg'=>'Unable to check if product is still being used', 'err'=>$rc['err']));
			}
			if( $rc['used'] != 'no' ) {
				return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2466', 'msg'=>"Product is still in use. " . $rc['msg']));
			}
		}
	}
	...
	return ciniki_core_objectDelete($ciniki, $args['business_id'], 'ciniki.writingcatalog.product', $args['product_id'], $uuid, 0x07);
}
?>
