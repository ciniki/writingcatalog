...
// <rsp stat="ok" />
//
function ciniki_writingcatalog_checkAccess(&$ciniki, $tnid, $method) {
    //
    ...
    $rc = ciniki_tenants_checkModuleAccess($ciniki, $tnid, 'ciniki', 'writingcatalog');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    $modules = $rc['modules'];
    ...
    if( !isset($rc['ruleset']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.writingcatalog.1', 'msg'=>'No permissions granted'));
    }
    ...
    // By default, fail
    //
    return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.writingcatalog.2', 'msg'=>'Access denied.'));
}
?>
